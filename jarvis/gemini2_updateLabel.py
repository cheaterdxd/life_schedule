...
class ReminderApp:
    def __init__(self, root_window):
        ...
        self.status_label = tk.Label(root_window, text="Đang chạy...", font=("Helvetica", 10), pady=5) # Giảm pady một chút
        self.status_label.pack(fill=tk.X, padx=10)

        # --- Thêm Label hiển thị thời gian kiểm tra cuối ---
        self.last_check_label = tk.Label(root_window, text="Chưa có lần kiểm tra nào.", font=("Helvetica", 9), fg="gray", pady=5)
    ...
    def save_log(self, answer):
        ...
        try:
            ...
            self.last_check_label.config(text=f"Lần cuối kiểm tra lúc: {timestamp}", fg="black") # Đổi lại màu chữ cho rõ hơn
            # ---------------------------------------------
        except Exception as e:
    ...
    def schedule_next_reminder(self):
        ...
        if self.is_running and time.time() >= self.sleep_until_time:
            ...
            # Giữ nguyên status_label là "Đang chạy..." hoặc hiển thị thời gian đến lần tiếp theo nếu muốn phức tạp hơn
            if self.countdown_id is None: # Chỉ cập nhật nếu không đang đếm ngược thời gian ngủ
            ...
            # Nếu đang ngủ, kiểm tra lại khi hết giờ ngủ
             ...
             # Lên lịch kiểm tra lại, cập nhật status_label ở update_countdown
    ...
    def go_to_sleep(self):
        ...
        if minutes:
            ...
            # self.schedule_next_reminder() # Không cần gọi ở đây nữa vì update_countdown sẽ gọi schedule_next_reminder khi hết giờ
    ...
    def update_countdown(self):
        ...
        if time.time() < self.sleep_until_time:
            ...
            print("Hết giờ ngủ.")
            # Sau khi thức dậy, lên lịch nhắc nhở tiếp theo
            self.schedule_next_reminder()
